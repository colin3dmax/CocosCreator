...
float dotPattern(vec2 p){
    ...
    float c = dot(sin(p/4. - cos(p.yx/.2 + iGlobalTime/4.)), vec2(.5));
...
void mainImage( out vec4 fragColor, in vec2 fragCoord ){
	...
	// Camera Setup.
    vec3 ro = camPath(iGlobalTime*1.5); // Camera position, doubling as the ray origin.
    vec3 lk = camPath(iGlobalTime*1.5 + .1);  // "Look At" position.
    vec3 lp = camPath(iGlobalTime*1.5 + 2.) + vec3(0, 2, 0); // Light position, somewhere near the moving camera.
