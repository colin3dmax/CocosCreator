...
struct repeatInfo
{
    vec3 sample; //object-space, cyclic
...
DF_out DF_composition( in vec3 pos )
{
    ...
    oHexA.d = DF_RoundedHex( infoA.sample-vec3(0,infoA.anchor.y,0),
    ...
    oHexB.d = DF_RoundedHex( infoB.sample-vec3(0,infoB.anchor.y,0),
	                         g_animationChannels.geometry_width*HEX_HALF_WIDTH, PRIM_HEIGHT );
...
vec4 MAT_apply(vec3 pos, TraceData traceData)
{
    ...
    float jitter_01 = SAMPLER_trilinear(pos*10.0+iGlobalTime*50.0);
...
Cam CAM_animate(vec2 uv, float fTime)
{
    ...
    cam.lens = 1.2+0.3*sin(iGlobalTime*0.1);
    cam.zoom = 3.0+sin(iGlobalTime*0.1)/cam.lens;
	return cam;
}
